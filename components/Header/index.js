...
// (Chỉnh sửa để nhận style từ parent hoặc animation variants)
const BackButton = ({ onClick }) => {
  return (
    <motion.button
      // Variants để nút này hiện ra chậm hơn menu 1 xíu cho đẹp
      ...
      whileTap={{ scale: 0.9 }}
      onClick={onClick}
      // Z-Index 50 để nổi lên trên các lớp sóng
      className="absolute top-[48px] right-[69px] z-50 cursor-pointer"
    >
      <img
...
const Header = ({ isOpen, setIsOpen, indexHeader }) => {
  ...
  return (
    <>
      ...
      <AnimatePresence mode="wait">
        {isOpen && (
          <div className="fixed inset-0 z-50 h-screen w-screen overflow-hidden">
            {/* 1. TEXT MENU */}
            <motion.div
              className="absolute inset-0 z-50 flex flex-col justify-center px-10 md:px-32 pointer-events-auto"
            ...
            {/* --- 2. BACK BUTTON (ĐÃ DI CHUYỂN VÀO ĐÂY) --- */}
            {/* Chỉ hiện nút Back khi không phải trang Home */}
      ...
      {/* HEADER BAR (Nút mở menu) */}
      <div className="px-[60px] py-[26px] flex flex-row justify-between items-center mob:px-[30px] relative z-40">
